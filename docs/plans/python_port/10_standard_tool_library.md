...
Port the standard-tool-library delta into a new
`week1_baseline/python/10_standard_tool_library` package, created as a copy of
the completed `08_the_repl_loop` step. Python has no step-9 port (per the
user's request — `09_global_executable` is gem/bin packaging with no Python
equivalent and is skipped entirely), so this single increment must absorb
everything that changed across *both* ruby `09_global_executable` and ruby
`10_standard_tool_library`, minus the parts of ruby step 9 that are
gem-packaging concerns.
...
there is no `boukensha_loader` concept. **Do not port `boukensha_loader.rb`,
`bin/boukensha`'s rc-file resolution, or any gem packaging** — this is also
why `09_global_executable` itself is not being ported at all, per the user's
instruction.

What *does* need porting is real ruby history that landed between steps 8–10,
confirmed by diffing the ruby directories directly (`diff
week1_baseline/ruby/08_the_repl_loop/lib/boukensha/client.rb
week1_baseline/ruby/10_standard_tool_library/lib/boukensha/client.rb`, etc.):

| Ruby file | Step | What changed |
|---|---|---|
| `lib/boukensha/client.rb` | 08→09 | The HTTP 401 special case added in Python/ruby step 8 is **removed** and stays removed in step 10 |
...
Confirmed locally: `week1_baseline/python/10_standard_tool_library` does not
exist yet (Python currently stops at `08_the_repl_loop`), and
`.boukensha/settings.yaml` currently declares exactly one `mcp_servers` entry
(`mud`, pointing at `week0_explore/mud_manager/bin/mud-manager --mcp`) — not
a `filesystem` entry. Verification steps below are written against that
actual config, not a hypothetical richer one.
...
### 0. Create the step-10 package

`week1_baseline/python/10_standard_tool_library` does not exist yet. Create it
as a copy of the completed `08_the_repl_loop` step (`cp -r
week1_baseline/python/08_the_repl_loop week1_baseline/python/10_standard_tool_library`),
then apply every change below in place — mirroring how `08_the_repl_loop`
itself started as a copy of `07_the_run_dsl`. Do **not** copy anything from a
Python `09_*` step, since none exists.
...
- `boukensha/config.py`: delete the `mud_host` / `mud_port` / `mud_username` /
  ...
  Match ruby's per-entry defaulting exactly (see
...
- Add `boukensha/mcp/__init__.py` (empty) and `boukensha/mcp/client.py` with a
  ...
  (`week1_baseline/ruby/10_standard_tool_library/lib/boukensha/mcp/client.rb`):
  - `Client.spawn(command, args=(), env=None)` classmethod / plain
    constructor that spawns the process with `subprocess.Popen`. Confirm
    ruby's exact `Open3.popen3(env, *cmd)` environment semantics before
    finalizing (whether the given hash *replaces* the child's environment or
    is layered on top of the parent's) and match it precisely in Python —
    silent divergence here breaks the MUD demo, since `mud-manager` needs
    `MUD_HOST`/`MUD_PORT`/etc from the spawned entry's `env:` block.
...
- Add `boukensha/tools/__init__.py` (empty) and `boukensha/tools/mcp.py`
  ...
  (`week1_baseline/ruby/10_standard_tool_library/lib/boukensha/tools/mcp.rb`):
  ...
  - `register_client(registry, client, prefix=None)`: reads existing names via
    ...
    is already taken, then calls
    `registry.tool(local, description=..., parameters=to_boukensha_params(schema))`
...
- `boukensha/__init__.py`: add a private `_register_mcp_servers(registry, cfg)`
  ...
  - For each, calls
    `tools.mcp.register(registry, command=entry["command"], args=entry["args"], env=entry["env"], prefix=entry["prefix"])`.
...
- Extract the provider/config-resolution block shared between `run` and
  ...
  working orchestration (same caution as the 08 plan's note on avoiding drift
  between `run()` and `repl()`).
...
- Add `examples/mcp_mud_demo.py`, a straight port of
  ...
  - `--dry` mode: spawn `week0_explore/mud_manager/bin/mud-manager --mcp`
    directly (the daemon itself is a ruby script — the Python MCP client only
    cares that it speaks stdio JSON-RPC, exactly like ruby's own comment:
    "exactly what the Python / Go / Rust / Java tracks do with their own
    SDKs") against a `MudManager::FakeMud`-equivalent. Since there is no
    Python `mud_manager` package, either (a) shell out to a small ruby
    one-liner that boots `MudManager::FakeMud` and prints its port, or (b)
    accept a `MUD_PORT`/etc set from an already-running fake MUD. Prefer (a)
    for a true one-command `--dry` smoke test; keep the process-management
    code isolated so it can be deleted without touching the MCP demo path if
    a Python `mud_manager` port ever exists.
...
- Replace the copied step-8 README with step-10 documentation: the MCP host
  ...
  `boukensha_loader`/`~/.boukensharc` narrative — Python has no loader, and
  Python's step 10 has no equivalent to ruby's `09_global_executable` at all.
...
- Add a `week1_baseline/python/10_standard_tool_library/test/` package using
  ...
  `skip` behavior. There is no Python analogue to `test_boukensha_loader.rb`
  — do not port it, consistent with skipping `09_global_executable` entirely.
...
week1_baseline/python/10_standard_tool_library/
  ...
  src/boukensha/
    ...
    run.py                              working_dir + _register_mcp_servers wiring
    mcp/
      __init__.py                       new
...
Note: the target lays out `src/boukensha/` (not a bare `boukensha/`) to match
this repo's actual Python package layout, confirmed against
`week1_baseline/python/08_the_repl_loop/src/boukensha/` — every prior Python
step uses a `src/` layout, so step 10 continues that convention even though
it is not called out explicitly in ruby's directory structure.
...
**Action**: Verify `08_the_repl_loop` is installed/importable before starting
this port — there is no Python `09_*` step to depend on instead.
...
3. Assert `Config._resolve_dir()` only honors `BOUKENSHA_DIR` then
   ...
   check locking in the step-8 revert).
...
10. Run the new launcher (and `examples/mcp_mud_demo.py --dry`) end-to-end
    against the repo's actual `.boukensha/settings.yaml`, which currently
    declares one `mud` MCP entry (`command:
    /home/system/claude-code-camp-2026-Q2/week0_explore/mud_manager/bin/mud-manager`,
    `args: [--mcp]`, `prefix: tbamud`), confirming the process boots without a
    paid provider call by exercising only the config/registration path (or
...
- The example and new MUD/MCP demo, README, launcher, and test suite
  ...
  `boukensha_loader` concepts that have no Python equivalent — and without a
  Python port of `09_global_executable`, which was intentionally skipped.

## Not Ported (out of scope for this step)

- Ruby's entire `09_global_executable` step (`Gemfile`, `boukensha.gemspec`,
  `bin/boukensha`, `lib/boukensha_loader.rb`,
  `test/test_boukensha_loader.rb`, `~/.boukensharc` resolution) — skipped per
  the user's explicit instruction; Python steps run via
  `week1_baseline/bin/python/NN_*` launchers, not an installed gem, so none of
  this has a Python equivalent.
- Any gem-packaging concern surfaced only by diffing `09_global_executable`
  against `10_standard_tool_library` (e.g. `Rakefile`, `Gemfile.lock`,
  `boukensha-0.10.0.gem`).
