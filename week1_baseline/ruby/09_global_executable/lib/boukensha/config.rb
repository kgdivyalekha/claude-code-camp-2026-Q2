  ...
  class Config
    ...
    def load_settings
      ...
      if ENV["BOUKENSHA_DEBUG"]
        puts "[boukensha] looking for settings at: #{settings_file}"
        puts "[boukensha] file exists? #{File.exist?(settings_file)}"
      end
      if File.exist?(settings_file)
        loaded = YAML.safe_load(File.read(settings_file)) || {}
        puts "[boukensha] loaded settings: #{loaded.inspect}" if ENV["BOUKENSHA_DEBUG"]
        loaded
      else
        puts "[boukensha] settings file not found, using empty hash" if ENV["BOUKENSHA_DEBUG"]
        {}
      end
    end
  end
