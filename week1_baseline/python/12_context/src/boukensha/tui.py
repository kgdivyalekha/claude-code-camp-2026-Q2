...
class Tui(App):
    ...
    def _render_progress(self) -> None:
        ...
        if self._live["active"]:
            ...
            cap = self._fmt_tokens(self._ctx.context_window)
            progress.update(
                f"[{color}]  [ready]   ctx {used} / {cap} ({pct}%)   "
                f"{self._turn_count} turns[/{color}]"
            )
    ...
    def _render_status(self) -> None:
        ...
        status.update(
            f" boukensha v{ver} · {model}  ·  ctx {used}/{cap} ({pct}%){ctx_indicator}·  "
            f"{tools} tools  ·  {clock} "
        )
