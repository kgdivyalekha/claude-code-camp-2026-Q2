...
Shared knowledge: the map, mobs, and shops below apply to every character
that plays this MUD, not just one login. Anyone reading `data/players/*.md`
for a specific character's state should also read this file for the world
itself. Update it regardless of which character discovered the room.
...
**Full confirmed route from Alchemist's Room back to Temple Square (walked
2026-07-18, no damage taken, no exp lost):**
Alchemist's Room → n (Narrow Passage) → n (South Stairs) → n (North Stairs)
→ w (The Hallway) → w (Statue's Room) → s (Bright Hallway) → s (Nexus) →
w (Dirty Hallway) → w (Beginning of the Passage) → s (Entrance to the
Newbie Zone) → w (Great Field, login room) → s (Great Field, outer) →
s (Behind the Temple Altar) → s (By the Temple Altar) → s (Temple of
Midgaard) → s (Temple Square). 15 moves total, all one-directional
corridors — no ambiguity once you know the sequence. Reverse it to go
back down to the Alchemist's Room from Temple Square.

- **Entrance to the Newbie Zone** — exits: n w. w → Great Field login room.
- **The Beginning of the Passage** — exits: e s. s → Entrance to Newbie Zone.
...
- **A Nexus** — e of Dirty Hallway; exits: (n-door) (e-door) s w.
  ...
  confirmed passed through without incident 2026-07-18). w → Dirty Hallway.
- **A Bright Hallway** — n of Nexus (open the door); exits: n s.
  "Interesting design on the floor" (unexamined). s → Nexus, n → Statue's Room.
- **The Statue's Room** — exits: e s. Statue with plaque. Talking newbie NPC.
  e → The Hallway, s → Bright Hallway.
- **The Hallway** (decorated) — exits: e w. e → North Stairs, w → Statue's Room.
  Someone's pet dragon wanders here too (harmless).
- **The North Stairs** — exits: s w u. u → Balcony (view of Dragonhelm Mts).
  w → The Hallway (confirmed open, NOT blocked). s → South Stairs.
- **The South Stairs** — s of North Stairs; exits: n s w u. w blocked (unlike
  North Stairs' w). n → North Stairs, s → Narrow Passage.
- **A Narrow Passage** — exits: n (s-oak door). n → South Stairs, s → oak
  door into Alchemist's Room. ⚠ **A zombiefied newbie is here and IS
  aggressive** (attacked us on approach 2026-07-18) — unlike the "VERY
  gaunt newbie" sighting which didn't aggro on an earlier pass. Fighting it
  briefly cost no HP and no exp change before we moved past/it disengaged;
  still, treat this room as a likely ambush spot and pass through quickly.
