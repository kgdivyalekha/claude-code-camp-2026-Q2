...
**Login:** queen  **Password:** password

## Current goal
**Reach level 7, then find and kill the minotaur.**
Side quest (2026-07-17): find "the red room".

## Character sheet (verified 2026-07-18)
- Level: 1 — Queen the Swordpupil
- HP: 24/24, Mana: 100/100, Move: 85/85 (full, after the walk back)
- Exp: 1037; needs 963 exp to reach level 2
- Gold: 96
...
- 2026-07-20: **Login attempt failed — the server no longer recognizes
  "queen"/"Queen" as an existing character.** Every fresh `init` (tried 4
  times, both lowercase "queen" and capitalized "Queen") produced the
  double "Please type Yes or No:" prompt. Diagnosed step by step: answering
  `no` gives "Okay, what IS it, then?" (tbaMUD's CON_NAME_CNFRM branch);
  answering `yes` gives "New character.\r\nGive me a password for Queen:" —
  this exact message is only sent by tbaMUD when `load_char()` fails to
  find a matching player file, i.e. the account genuinely does not exist
  on the server right now (not a take-over prompt — those only ever
  showed up for *live* characters like horseman, which reconnected
  instantly with "Reconnecting." and no name/password check at all). Did
  NOT proceed past the password prompt — closed the socket immediately
  each time so no new level-1 "Queen" was created (would've been a
  different character with no relation to this file's history). Could not
  check hunger/thirst status this session because login never completed.
  **Needs investigation**: was the queen player file lost/reset on the
  server? Until resolved, future sessions should expect this same failure
  and should not create a fresh character to replace it without asking.
- 2026-07-18: Reconnected and found the character deep in the Newbie Zone
  dungeon area, in The Alchemist's Room (near the level-7 minotaur
  stairway), full HP, 1037 exp, 96 gold. Walked the full route back to
  Temple Square (15 moves — see world.md for the exact confirmed path).
  A zombiefied newbie in A Narrow Passage attacked on the way through but
  no HP/exp was lost; a pet dragon in The Hallway/Nexus was harmless.
  Arrived at Temple Square with full HP and full movement. No fighting
  done this session, no exp gained/spent — pure navigation.
